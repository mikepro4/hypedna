...
class ClipsTimeline extends Component {
	...
	onMouseUp = event => {
		if (this.state.startedDragging) {
			...
			this.props.optimisticTrackUpdate(
				_.assign({}, this.props.track, {
					clips: this.state.updatedChannelClips
				})
			);
			this.props.updateTrackClips(
				this.props.video.googleId,
				this.props.track._id,
				this.state.updatedChannelClips,
				() => {
	...
	onMouseMove = event => {
		...
		if (this.state.startedDragging) {
			...
			this.props.track.clips = this.getUpdatedTrackClips(
				{
					start: newClipStart,
					end: newClipEnd
				},
		...
		if (this.state.startedMoving) {
			// console.log("moving clip: ", this.props.selectedClip._id);
			const filteredClipArray = _.filter(this.originalClips, clip => {
				return clip._id == this.props.selectedClip._id;
			});
			// console.log(filteredClipArray[0]);
			// console.log(newClipStart, newClipEnd);

			let newMovedClip = {};
			let diff = newClipEnd - newClipStart;
			...
			// console.log(newMovedClip);

			let cliptoUpdateIndex = _.findIndex(this.originalClips, {
				_id: this.props.selectedClip._id
			});

			let newClipsArray = update(this.originalClips, {
			...
			// this.props.optimisticTrackUpdate(
			// 	_.assign({}, this.props.track, {
			// 		clips: this.getUpdatedTrackClips(newMovedClip, newClipsArray)
			// 	})
			// );

			// this.props.optimisticTrackUpdate(
			// 	this.getUpdatedTrackClips(newMovedClip, newClipsArray)
			// );
		}
	};
	...
	getUpdatedTrackClips = (newClip, clipsArray) => {
		...
		let filteredClips = _.filter(clipsArray, clip => {
			if (
				!this.state.startedDragging &&
